...
<head>
    ...
    <title>Design Your Custom Hoodie | Stitch Smart</title>
    ...
    <style>
        ...
        .color-circle { width: 24px; height: 24px; border-radius: 50%; display: inline-block; margin: 5px; cursor: pointer; border: 1px solid var(--border-color); }
        .color-circle.selected { border: 2px solid var(--accent-bronze, #ca9745); }
        .image-preview, .dynamic-image { max-width: 100%; height: auto; margin-top: 10px; border: 1px solid var(--border-color); border-radius: 8px; }
        .size-chart table { width: 100%; border-collapse: collapse; }
        .size-chart th, .size-chart td { border: 1px solid var(--border-color); padding: 8px; text-align: center; }
        .template-gallery img { max-width: 160px; object-fit: contain; border: 1px solid var(--border-color); border-radius: 6px; }
    </style>
</head>
...
<div class="container my-5">
        ...
        <div id="step3" class="step card p-4">
            ...
            <div class="row mt-4">
                <div class="col-md-6">
                    ...
                    <a href="#" 
                       class="btn btn-secondary mt-1" onclick="downloadCustomizedTemplate(event)">
                       Download Customized Template Pack
                    </a>
        ...
        <div id="step4" class="step card p-4">
            ...
            <div class="row">
                <div class="col-md-4">
                    ...
                    <img id="sunfadeImage" class="dynamic-image mt-3" src="<?= BASE_URL ?>/pictures/design/empty_hoodie.png" alt="Sunfade Preview">
                </div>
                <div class="col-md-4">
                    <h5>Stitching</h5>
                    <div class="form-check">
                    ...
                    <img id="stitchingImage" class="dynamic-image mt-3" src="<?= BASE_URL ?>/pictures/design/empty_hoodie.png" alt="Stitching Preview">
                </div>
                <div class="col-md-4">
                    <h5>Distressing</h5>
                    <div class="form-check">
                    ...
                    <img id="distressingImage" class="dynamic-image mt-3" src="<?= BASE_URL ?>/pictures/design/empty_hoodie.png" alt="Distressing Preview">
                </div>
            </div>
    ...
    <script>
        ...
        const sunfadeImages = {
            'Shoulder Sunfade': '<?= BASE_URL ?>/pictures/design/Shoulder Sunfade (H).png?v=<?= time() ?>',
            'Shoulder & Bottom Sunfade': '<?= BASE_URL ?>/pictures/design/Shoulder_Bottom_Sunfade_H.png?v=<?= time() ?>',
            'Circular Sunfade': '<?= BASE_URL ?>/pictures/design/Circular Sunfade (H).png?v=<?= time() ?>',
            'All-over Sunfade': '<?= BASE_URL ?>/pictures/design/All-over Sunfade (H).png?v=<?= time() ?>'
        };
        ...
        function applyColorToPreviews(color) {
            ...
            const imagesToColor = document.querySelectorAll('.dynamic-image, .template-gallery img');
        ...
        function downloadCustomizedTemplate(e) {
            e.preventDefault();
            ...
            // Draw base color
            ctx.fillStyle = orderData.color || '#FFFFFF';
            ctx.fillRect(0, 0, canvas.width, canvas.height);
            
            const imagesToDraw = [];
            imagesToDraw.push('<?= BASE_URL ?>/pictures/design/empty_hoodie.png');
            
            const sunfadeOpt = document.querySelector('input[name="sunfadeOption"]:checked')?.value;
            if (sunfadeOpt && sunfadeOpt !== 'None' && sunfadeImages[sunfadeOpt]) imagesToDraw.push(sunfadeImages[sunfadeOpt]);
            
            const stitchingOpt = document.querySelector('input[name="stitchingOption"]:checked')?.value;
            if (stitchingOpt && stitchingOpt !== 'None' && stitchingImages[stitchingOpt]) imagesToDraw.push(stitchingImages[stitchingOpt]);
            
            const distressingOpt = document.querySelector('input[name="distressingOption"]:checked')?.value;
            if (distressingOpt && distressingOpt !== 'None' && distressingImages[distressingOpt]) imagesToDraw.push(distressingImages[distressingOpt]);
            
            let loadedCount = 0;
            const loadedImages = [];
            
            imagesToDraw.forEach((src, index) => {
                const img = new Image();
                ...
                img.onload = () => {
                    ...
                    if (loadedCount === imagesToDraw.length) {
                        loadedImages.forEach(imageObj => {
                            if(imageObj) {
                                if (imageObj.src.includes('empty_hoodie')) {
                        ...
                        if (orderData.comments) {
                            ...
                            ctx.font = '20px Arial';
                            ctx.textAlign = 'left';
                            ctx.fillText('Comments:', 10, canvas.height - 60);
                            ctx.font = '16px Arial';
                            ctx.fillText(orderData.comments.substring(0, 50) + '...', 10, canvas.height - 30);
                        }
                        
                        const a = document.createElement('a');
                        a.href = canvas.toDataURL('image/png');
                        a.download = 'Customized_Hoodie_Template.png';
                ...
                img.onerror = () => {
                    ...
                    if (loadedCount === imagesToDraw.length) {
                        btn.innerHTML = originalText;
                        btn.style.pointerEvents = 'auto';
                        alert("Could not load all template parts.");
                    }
                };
            });
        }
        ...
        function validateStep(step) {
            ...
            if (step === 1) {
                ...
                checkField(orderData.color, 'Please select a Color.', document.getElementById('colorDropdown') || document.getElementById('customColor'));
            } else if (step === 5) {
...
Sent via Hoodie Custom Form
            ...
            formData.append('subject', `New Hoodie Inquiry from ${name}`);
            ...
            if (designImage && designImage.files[0]) {
                formData.append('designImage', designImage.files[0]);
            }
